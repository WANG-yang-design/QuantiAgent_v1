...
import { SystemBar, fmtWan } from "../components/Common";
              ...
              {gitems.map((i) => {
                ...
                return (
                  <tr key={i.symbol} className="cursor-pointer hover:bg-gray-50" onClick={() => nav(`/symbol/${i.symbol}`)}>
                    ...
                    <td className="td"><span className={`badge ${i.asset_type === "stock" ? "bg-purple-50 text-purple-700" : "bg-cyan-50 text-cyan-700"}`}>{i.asset_type === "stock" ? "股票" : "ETF"}</span></td>
                    ...
                    <td className="td" onClick={(e) => e.stopPropagation()}>
                      <div className="flex gap-1 items-center">
                        <button className="btn-ghost" title="设为主动监控"
                          onClick={() => setCats.mutate({ symbol: i.symbol, cats: [...i.categories, "watched"] })}>
                          <Star size={13} />
                        </button>
