...
class AkShareClient(BaseDataSource):
    ...
    def get_news(self, symbol: str, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for _, r in df.head(limit).iterrows():
                rows.append({
                    "news_id": f"{self.name}_{symbol}_{_safe_str(r.get('关键词'))}_{r.name}",
                    "symbol": symbol,
                    "title": _safe_str(r.get("新闻标题")),
                    "content": _safe_str(r.get("新闻内容")),
                    "publish_time": r.get("发布时间"),
                    "source": "eastmoney",
    ...
    def get_announcements(self, symbol: str, limit: int = 20) -> List[Dict[str, Any]]:
        rows: List[Dict[str, Any]] = []
        try:
            df = self.ak.stock_notice_report(symbol="全部", date=datetime.now().strftime("%Y%m%d"))
    ...
    def get_sentiment(self, symbol: str, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for _, r in df.head(limit).iterrows():
                rows.append({
                    "record_id": f"guba_{symbol}_{r.name}",
                    "symbol": symbol,
                    "platform": "guba",
                    "content": _safe_str(r.get("标题")),
    ...
    def get_etf_info(self, symbol: str) -> Dict[str, Any]:
        ...
        info = {
            ...
            "fee_rate": 0.0,
            "fund_company": "",
            "is_qdii": symbol.startswith("1599"),   # 纳指/标普等跨境 ETF 代码特征(粗略)
        }
